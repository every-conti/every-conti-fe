...
export default function ContiRowCard({
  ...
  // 2) 기존/현재 songIds 준비
  const initialSongIds = useMemo(() => conti.songs.map((cs) => cs.song.id), [conti.songs]);
        ...
        <div className="flex flex-col sm:flex-row">
          ...
          {playSongs.length > 0 ? (
            <PlayButton songs={[...playSongs]}>
              <div className="relative w-full sm:w-57 sm:h-auto flex-shrink-0">
                <ImageWithFallback
                  src={firstThumb}
                  alt={conti.title}
                  ...
                  <div className="flex flex-col gap-1">
                    {conti.songs.slice(0, 3).map((s, i) => (
                      <div key={s.song.id} className="text-xs text-gray-600 flex justify-between">
                        ...
                        <span className="text-gray-400">{parseSongDuration(s.song.duration)}</span>
                      </div>
                    ))}
                    {conti.songs.length > 3 && (
                      <p className="text-xs text-gray-400">
                        외 {conti.songs.length - 3}곡 더 있음…
                      </p>
                    )}
                  </div>
                ) : (
        ...
        <DialogContent className="sm:max-w-2xl" onClick={(e) => e.stopPropagation()}>
          ...
          <Tabs defaultValue="info">
            ...
            <TabsContent value="songs" className="mt-4" onClick={(e: any) => e.stopPropagation()}>
              <SongPickerAndList value={editSongs} onChange={(songs) => setEditSongs(songs)} />
