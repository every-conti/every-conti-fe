...
export default function PlayerBar() {
  ...
  const { isPlaying, setIsPlaying, playlist, setPlayList, currentSongIndex, setCurrentSongIndex } =
    usePlayerStore();
  ...
  const hasPrev = shuffle ? history.length > 0 : currentSongIndex !== null && currentSongIndex > 0;
  ...
  return (
    <>
      {isFullScreen ? (
        <FullScreenMusicPlayer {...musicPlayerPropsDto} />
      ) : (
        <SimpleBottomBar {...musicPlayerPropsDto} />
      )}
