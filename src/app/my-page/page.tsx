...
import { Edit3, Music, Shield, LogOut } from "lucide-react";
...
function MyPage() {
  ...
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isLoading } =
    useInfiniteMyContiQuery({ memberId: user?.id }, { enabled: Boolean(user?.id) });
  ...
  const onUpdated = (updated: ContiWithSongDto) => {
    queryClient.setQueryData<any>(["myContis", user?.id], (oldData: any) => {
      ...
      return {
        ...
        pages: oldData.pages.map((page: any) => ({
          ...page,
          items: page.items.map((c: ContiWithSongDto) => (c.id === updated.id ? updated : c)),
        })),
      };
    });
  };
  ...
  const handleSubmitPw = async () => {
    ...
    try {
      ...
      const message =
        e?.message ?? (typeof e === "string" ? e : "비밀번호 변경 중 오류가 발생했어요.");
      toast.error(message);
    } finally {
      setChangingPw(false);
      setIsPwOpen(false);
        ...
        <div className="max-w-6xl mx-auto px-6 py-8">
          <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
            <TabsList className="mb-8">
              ...
              <TabsTrigger value="contis">내 콘티</TabsTrigger>
            </TabsList>
      ...
      <Dialog open={isDeleteAccountOpen} onOpenChange={setIsDeleteAccountOpen}>
        <DialogContent className="sm:max-w-md" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="space-y-3 text-sm text-gray-700">
            <p>
              계정을 삭제하면 모든 데이터가 <span className="font-medium">영구적으로 삭제</span>
              됩니다. 이 작업은 되돌릴 수 없습니다.
            </p>
            <div className="space-y-2">
              <Label htmlFor="delete-confirm">확인 문구를 입력하세요</Label>
              <p className="text-xs text-gray-500">
                아래 입력창에 <span className="font-medium">“{expectedPhrase}”</span>를 정확히
                입력하면 삭제가 진행됩니다.
              </p>
              <Input
                id="delete-confirm"
      ...
      <Dialog open={isPwOpen} onOpenChange={setIsPwOpen}>
        <DialogContent className="sm:max-w-md" onClick={(e) => e.stopPropagation()}>
          ...
          <DialogFooter>
            ...
            <Button type="button" onClick={handleSubmitPw} disabled={changingPw}>
              {changingPw ? "변경 중…" : "변경"}
            </Button>
          </DialogFooter>
        </DialogContent>
