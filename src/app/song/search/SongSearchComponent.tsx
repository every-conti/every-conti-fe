...
import { useSongPropertiesQuery, useInfiniteSearchSongQuery } from "src/app/api/song";
...
export default function SongSearchComponent() {
  ...
  const [selectedSongType, setSelectedSongType] = useState<SongTypeTypes | null>(null);
  ...
  const normalize = (s: string) => s.split("&").filter(Boolean).sort().join("&");
  // State -> URL
  ...
  const ready =
    !!searchSongProperties && normalize(serialized) === normalize(searchParams.toString());
  ...
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isLoading, isError } =
    useInfiniteSearchSongQuery(queryParams, { enabled: ready });
  ...
  useEffect(() => {
    ...
    const themeIds = themeIdsRaw ? themeIdsRaw.split(",") : [];
    ...
    setSelectedPraiseTeam(
      praiseTeam
        ? (searchSongProperties?.praiseTeams.find(
            (t) => t.praiseTeamName === praiseTeam
          ) as PraiseTeamDto)
        : null
    );
    setSelectedSeason(
      season
        ? (searchSongProperties?.seasons.find((s) => s.seasonName === season) as SongSeasonDto)
        : null
    );
    setDuration([minDuration ?? MIN_SONG_DURATION, maxDuration ?? MAX_SONG_DURATION]);

    setSelectedBible(
      bible ? (searchSongProperties?.bibles.find((b) => b.bibleKoName === bible) as BibleDto) : null
    );
  ...
  return (
    <>
      <PageTitle
        title="찬양 검색"
        description="원하는 찬양을 필터를 통해 찬양팀, 장르, 주제 별로 찾아보세요"
      />
            ...
            <div className="space-y-4">
              {songs.map((song, idx) => (
                <div key={song.id}>
                  <WorshipSearchCard
                    song={song}
                    onAddModalBtnClick={onAddModalBtnClick}
                    onLyricsModalClick={onLyricsModalClick}
                  />
                </div>
              ))}
      ...
      <LyricsModal open={lyricsOpen} onOpenChange={setLyricsOpen} song={modaledSong} />
    </>
  );
}
